...
from ..dependencies import get_conn, get_current_user_dependency
...
async def create_user(
    user_data: UserCreate, conn: aiomysql.Connection = Depends(get_conn)
):
    """创建新用户"""
    try:
        # 数据验证
        if not validate_password(user_data.password):
            raise HTTPException(
                status_code=400, detail="密码强度不够，至少6位且包含字母和数字"
            )
...
async def update_self(
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            # 检查用户是否存在
            await cursor.execute(
                "SELECT id FROM users WHERE id = %s", (current_user["id"],)
            )
...
async def update_user(
    user_id: int, user_data: UserUpdate, conn: aiomysql.Connection = Depends(get_conn)
):
    """更新用户信息"""
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
...
async def change_password(
    ...
    try:
        if not validate_password(password_data.new_password):
            raise HTTPException(
                status_code=400, detail="新密码强度不够，至少6位且包含字母和数字"
            )

        async with conn.cursor(aiomysql.DictCursor) as cursor:
            # 验证旧密码
            await cursor.execute(
                "SELECT hashed_password FROM users WHERE id = %s", (current_user["id"],)
            )
            user = await cursor.fetchone()
            user_id = current_user["id"]
            if not user:
            ...
            return JSONResponse(status_code=200, content={"message": "密码修改成功"})
    except HTTPException:
        raise
    except Exception as e:
        await conn.rollback()
...
async def reset_user_password(
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            return JSONResponse(status_code=200, content={"message": "密码重置成功"})
    except HTTPException:
        raise
    except Exception as e:
        await conn.rollback()
...
async def toggle_user_status(
    user_id: int, conn: aiomysql.Connection = Depends(get_conn)
):
    """切换用户状态（启用/禁用）"""
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
...
async def get_user_stats(
    ...
    try:
        ...
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            # 总借阅次数
            await cursor.execute(
                "SELECT COUNT(*) as total FROM borrows WHERE user_id = %s", (id,)
            )
            total_borrows = (await cursor.fetchone())["total"]

            # 当前借阅数
            await cursor.execute(
                "SELECT COUNT(*) as active FROM borrows WHERE status = 'borrowed' AND user_id = %s",
                (id,),
            )
            ...
            # 逾期借阅数
            await cursor.execute(
                "SELECT COUNT(*) as overdue FROM borrows WHERE status = 'overdue' AND user_id = %s",
                (id,),
            )
...
async def get_statistics(conn: aiomysql.Connection = Depends(get_conn)):
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            # 当前借阅数
            await cursor.execute(
                "SELECT COUNT(*) as active_borrows FROM borrows WHERE status = 'borrowed'"
            )
            active_borrows = (await cursor.fetchone())["active_borrows"]

            # 逾期借阅数
            await cursor.execute(
                "SELECT COUNT(*) as overdue_borrows FROM borrows WHERE status = 'overdue'"
            )
...
async def batch_delete_users(
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            # 检查用户是否存在
            await cursor.execute(
                "SELECT id FROM users WHERE id IN %s", (tuple(user_ids),)
            )
